...
describe("ANGELVesting", function () {
  let angelToken;
  let angelVesting;
  let owner;
  let beneficiary1;
  ...
  const INITIAL_SUPPLY = ethers.parseEther("10000000000"); // 10B ANGEL
  const VESTING_AMOUNT = ethers.parseEther("1000000"); // 1M ANGEL
  ...
  beforeEach(async function () {
    ...
    angelToken = await ANGEL.deploy();

    // Deploy ANGELVesting
    const ANGELVesting = await ethers.getContractFactory("ANGELVesting");
    angelVesting = await ANGELVesting.deploy(angelToken.target);

    // Transfer tokens to vesting contract
    await angelToken.transfer(angelVesting.target, VESTING_AMOUNT * 10n);
  });

  describe("Deployment", function () {
    it("Should set the correct token address", async function () {
      expect(await angelVesting.angelToken()).to.equal(angelToken.target);
    });

    it("Should set the correct owner", async function () {
      expect(await angelVesting.owner()).to.equal(owner.address);
    });

    it("Should have correct vesting parameters", async function () {
      expect(await angelVesting.CLIFF_DURATION()).to.equal(CLIFF_DURATION);
      expect(await angelVesting.VESTING_DURATION()).to.equal(VESTING_DURATION);
      expect(await angelVesting.TOTAL_DURATION()).to.equal(TOTAL_DURATION);
    });
  });

  describe("Creating Vesting", function () {
    it("Should create vesting successfully", async function () {
      const tx = await angelVesting.createVesting(beneficiary1.address, VESTING_AMOUNT);
      const receipt = await tx.wait();

      expect(receipt).to.emit(angelVesting, "VestingCreated");

      const vesting = await angelVesting.getVesting(beneficiary1.address);
    ...
    it("Should fail when creating vesting for zero address", async function () {
      await expect(
        angelVesting.createVesting(ethers.ZeroAddress, VESTING_AMOUNT)
      ).to.be.revertedWith("Invalid beneficiary");
    });

    it("Should fail when allocation is zero", async function () {
      await expect(
        angelVesting.createVesting(beneficiary1.address, 0)
      ).to.be.revertedWith("Allocation must be > 0");
    });

    it("Should fail when vesting already exists", async function () {
      await angelVesting.createVesting(beneficiary1.address, VESTING_AMOUNT);

      await expect(
        angelVesting.createVesting(beneficiary1.address, VESTING_AMOUNT)
    ...
    it("Should fail when not enough tokens in contract", async function () {
      ...
      await expect(
        angelVesting.createVesting(beneficiary1.address, hugeAmount)
      ).to.be.revertedWith("Insufficient tokens in contract");
    });

    it("Should fail when called by non-owner", async function () {
      await expect(
        angelVesting.connect(beneficiary1).createVesting(beneficiary2.address, VESTING_AMOUNT)
  ...
  describe("Claiming Tokens", function () {
    ...
    it("Should not allow claiming before cliff", async function () {
      await expect(
        angelVesting.connect(beneficiary1).claim()
    ...
    it("Should allow claiming after cliff", async function () {
      ...
      const claimable = await angelVesting.claimableAmount(beneficiary1.address);
      expect(claimable).to.be.gt(0);

      await angelVesting.connect(beneficiary1).claim();
    ...
    it("Should allow multiple claims", async function () {
      ...
      const claimable1 = await angelVesting.claimableAmount(beneficiary1.address);
      await angelVesting.connect(beneficiary1).claim();

      // Second claim after another 10 months
      await time.increase(10 * 30 * 24 * 60 * 60);
      const claimable2 = await angelVesting.claimableAmount(beneficiary1.address);
      await angelVesting.connect(beneficiary1).claim();
  ...
  describe("Revoking Vesting", function () {
    ...
    it("Should revoke vesting successfully before cliff", async function () {
      const ownerBalanceBefore = await angelToken.balanceOf(owner.address);

      await expect(angelVesting.revoke(beneficiary1.address))
        .to.emit(angelVesting, "VestingRevoked")
        .withArgs(beneficiary1.address, 0, VESTING_AMOUNT);

      const vesting = await angelVesting.getVesting(beneficiary1.address);
      expect(vesting.revoked).to.equal(true);

      const ownerBalanceAfter = await angelToken.balanceOf(owner.address);
    ...
    it("Should fail when vesting doesn't exist", async function () {
      await expect(
        angelVesting.revoke(beneficiary2.address)
      ).to.be.revertedWith("No vesting found");
    });
    ...
    it("Should fail when already revoked", async function () {
      ...
      await expect(
        angelVesting.revoke(beneficiary1.address)
      ).to.be.revertedWith("Already revoked");
    });

    it("Should fail when called by non-owner", async function () {
      await expect(
        angelVesting.connect(beneficiary1).revoke(beneficiary1.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
  ...
  describe("View Functions", function () {
    ...
    it("Should return correct locked amount", async function () {
      const locked = await angelVesting.lockedAmount(beneficiary1.address);
      expect(locked).to.equal(VESTING_AMOUNT);

      // After cliff + 25% vesting
      await time.increase(CLIFF_DURATION + VESTING_DURATION / 4);
      const lockedAfter = await angelVesting.lockedAmount(beneficiary1.address);
    ...
    it("Should return correct vesting progress", async function () {
      expect(await angelVesting.vestingProgress(beneficiary1.address)).to.equal(0);

      await time.increase(CLIFF_DURATION + VESTING_DURATION / 2);
      expect(await angelVesting.vestingProgress(beneficiary1.address)).to.equal(50);

      await time.increase(VESTING_DURATION / 2);
      expect(await angelVesting.vestingProgress(beneficiary1.address)).to.equal(100);
    });

    it("Should return correct weeks left", async function () {
      const weeksTotal = Math.floor(TOTAL_DURATION / (7 * 24 * 60 * 60));
      const weeksLeft = await angelVesting.weeksLeft(beneficiary1.address);
      expect(Number(weeksLeft)).to.be.closeTo(weeksTotal, 2);

      await time.increase(TOTAL_DURATION);
      expect(await angelVesting.weeksLeft(beneficiary1.address)).to.equal(0);
    });

    it("Should return correct next unlock time", async function () {
      const vesting = await angelVesting.getVesting(beneficiary1.address);
      expect(await angelVesting.nextUnlockTime(beneficiary1.address)).to.equal(vesting.cliffEnd);

      await time.increase(CLIFF_DURATION + 1);
      const currentTime = await time.latest();
      expect(await angelVesting.nextUnlockTime(beneficiary1.address)).to.equal(currentTime);
    });
  });
  ...
  describe("Pause Functionality", function () {
    ...
    it("Should allow owner to pause", async function () {
      await angelVesting.pause();
      expect(await angelVesting.paused()).to.equal(true);
    });
    ...
    it("Should allow owner to unpause", async function () {
      await angelVesting.pause();
      await angelVesting.unpause();
      expect(await angelVesting.paused()).to.equal(false);
