...
/**
 * @title CAPXVesting
 * @dev Vesting contract for CAPX tokens with cliff and linear vesting
 *
 * Architecture:
 ...
 */
contract CAPXVesting is Ownable, ReentrancyGuard, Pausable {
    ...
    // CAPX token
    IERC20 public immutable capxToken;
    ...
    // Total tokens allocated across all vestings
    ...
    /**
     * @dev Constructor
     * @param _capxToken Address of CAPX token contract
     */
    constructor(address _capxToken) {
        require(_capxToken != address(0), "Invalid token address");
    ...
    function createVesting(
        ...
        require(
            capxToken.balanceOf(address(this)) >= totalAllocated + allocation,
    ...
    function claim() external nonReentrant whenNotPaused {
        ...
        capxToken.safeTransfer(msg.sender, claimable);
    ...
    function revoke(address beneficiary) external onlyOwner {
        ...
        if (unvested > 0) {
            ...
            capxToken.safeTransfer(owner(), unvested);
        }
    ...
    function recoverTokens(address token, uint256 amount) external onlyOwner {
        require(token != address(capxToken), "Cannot recover vesting tokens");
        IERC20(token).safeTransfer(owner(), amount);
    }

    /**
     * @dev Get total available tokens in contract (for admin dashboard)
     * @return Available token balance
     */
    function availableTokens() external view returns (uint256) {
        return capxToken.balanceOf(address(this));
    }
}
